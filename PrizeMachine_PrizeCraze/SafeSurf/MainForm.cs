...
namespace SCTV
{
    ...
    public partial class MainForm : Form
    {
        ...
        ExtendedWebBrowser mainBrowser;
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                mainBrowser = this._windowManager.New();
                mainBrowser.DocumentCompleted += MainBrowser_DocumentCompleted;
                mainBrowser.Url = new Uri("https://www.prizecraze.com/");
                mainBrowser.StartNewWindow += mainBrowser_StartNewWindow;

                try
                {
                    mainBrowser.ObjectForScripting = new GetMainBrowserDocumentString();
                }
                catch (Exception ex)
                {
                    throw;
        ...
        {
            if (!((HtmlElement)sender).OuterHtml.Contains("id=\"submitLogin\""))
            {
                if (!loggingIn)
                {
                    ...
                    refreshUtilities.GoToURL("https://www.prizecraze.com/", lblRefreshTimer, mainBrowser);
                }
                else
        ...
        private void mainBrowser_StartNewWindow(object sender, BrowserExtendedNavigatingEventArgs e)
        {
            ...
            else
            {
                ...
                //tourBrowser.DocumentCompleted += tour_DocumentCompleted;
                newBrowser.Downloading += TourBrowser_Downloading;
                newBrowser.DownloadComplete += TourBrowser_DownloadComplete;
                newBrowser.Navigating += TourBrowser_Navigating;
        ...
        private void MainBrowser_DocumentCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
        {
            try
            {
                ...
                if (mainBrowser.Url != null)
                {
                    if(documentString.Contains("</i>&nbsp;Login</a>"))//need to login
                    {
                    ...
                    else if (mainBrowser.Url.ToString().ToLower() == "https://www.prizecraze.com" || mainBrowser.Url.ToString().ToLower() == "https://www.prizecraze.com/" || mainBrowser.Url.ToString().ToLower().StartsWith("https://www.prizecraze.com/?"))//this is the first page
                    {
                        loggingIn = false;
                        ...
                        if(!foundPrize)
                            iteratePrizes(mainBrowser.DocumentText);
                    }
                    else if (mainBrowser.Url.ToString().ToLower().Contains("https://www.prizecraze.com/prize/details/"))//this is the second page
                    {
                        if (foundPrize && !enteredTheContest)
                            enteredTheContest = enterContest(mainBrowser.Document);
                    }
                }
            }
            catch (Exception ex)
        ...
        {
            //<div class="prize-card   DESKTOP PORTRAIT" style="background:url(//static.prizecraze.com/Images/aqua-gradient.png);">
            string splitString = "<div class=\"prize-card   DESKTOP ";
            ...
            foreach (string prize in prizes)
            {
                ...
                //if (!prize.Contains("$25 Daily Cash"))//skip the first one right now because there is an error on their site
                //{
                //<span>10 Entries Remaining</span>
                entriesLeftString = findValue(prize, "<span>", " Entries Remaining");
                    //entriesLeftString = findValue(entriesLeftString, "<span class=\"hidden-xs\">", " ");

                    if (int.TryParse(entriesLeftString, out entriesLeft) && entriesLeft > 0)
                    {
                        string urlToGoTo = "https://www.prizecraze.com/prize/details/" + findValue(prize, "<a href=\"/prize/details/", "\"");
                        ...
                        //{
                        refreshUtilities.Cancel();
                        refreshUtilities.GoToURL(urlToGoTo, lblRefreshTimer, mainBrowser);
            ...
            if (entriesLeft == 0 && !switchingUsers && areWeFinished())
                switchUsers();
        }

        private bool areWeFinished()
        {
            //<div class="text-center progress-text russo">110 of 110 possible entries completed today.</div>

            bool finished = false;

            string entriesTodayString = findValue(mainBrowser.DocumentText, "progress-text russo\"", " entries completed");
            string entriesString = findValue(entriesTodayString, ">", " of ");
            string totalPossible = findValue(entriesTodayString, " of ", " possible");

            finished = entriesString == totalPossible;

            return finished;
        }

        private bool enterContest(HtmlDocument pageDocument)
        {
            if (!enteredTheContest)
            {
                HtmlElementCollection elc = pageDocument.GetElementsByTagName("button");

                foreach (HtmlElement el in elc)
                {
                    if (el.InnerText != null && el.InnerText.Trim() == "ENTER NOW")
                    {
        ...
        private bool findQuickPick(HtmlDocument pageDocument)
        {
            if (!foundQuickPick)
            {
                ...
                foreach (HtmlElement el in elc)
                {
                    if (el.InnerText != null && (el.InnerText.Trim() == "QUICK PICK" || el.InnerText.Trim() == "AUTO PICK"))
                    {
                        refreshUtilities.ClickElement(el, lblRefreshTimer);
                        foundQuickPick = true;
        ...
        private bool findNextContestLink(string pageContent)
        {
            ...
            if (nextContestLink.Trim().Length > 0)
            {
                nextContestLink = "http://www.rewardit.com/" + nextContestLink;
                ...
                refreshUtilities.GoToURL(nextContestLink, lblRefreshTimer, mainBrowser);
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            enterContest(mainBrowser.Document);
        }
        ...
        [ComVisible(true)]
        public class GetMainBrowserDocumentString
        {
            public void CallServerSideCode()
            {
                try
                {
                    ...
                    var doc = currentForm.mainBrowser.Document;
        ...
        private void logout()
        {
            refreshUtilities.GoToURL("https://www.prizecraze.com/Home/Logout", lblRefreshTimer, mainBrowser);
        ...
        private void login()
        {
            ...
            if (users.Count > 0)
            {
                ...
                HtmlElementCollection elc = mainBrowser.Document.GetElementsByTagName("a");

                foreach (HtmlElement el in elc)
                {
                    if (el.InnerText != null && el.InnerText.Contains(" Login"))
                    {
        ...
        private void populateUsernamePassword()
        {
            ...
            HtmlElementCollection elc = mainBrowser.Document.GetElementsByTagName("input");
            ...
            {
                //click enter now
                //<button id="submitLogin" class="btn btn-warning btn-text" style="font-size:20px;font-weight:800;">ENTER NOW</button>

                elc = mainBrowser.Document.GetElementsByTagName("button");
        ...
        {
            string docString = mainBrowser.DocumentText;
