...
namespace SCTV
{
    ...
    public class WindowManager
    {
        ...
        /// </summary>
        public void Close(int tabIndexToClose)
        {
            if (this._tabControl.TabPages.Count - 1 >= tabIndexToClose)
            {
                // Find the active page
                TabPageEx page = (TabPageEx)this._tabControl.TabPages[tabIndexToClose];
                // Check wheter there is actually a page selected
                if (page != null)
                {
                    // Remove the page
        ...
        public BrowserControl ActiveBrowserControl
        {
            get
            {
                try
                {
                    if (_tabControl != null && _tabControl.TabCount > 0 && _tabControl.SelectedIndex >= 0)
                    {
                        TabPageEx page = (TabPageEx)_tabControl.SelectedTab;
                        if (page != null)
                        {
